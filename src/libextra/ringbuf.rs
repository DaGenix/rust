...
fn grow<T>(nelts: uint, loptr: &mut uint, elts: &mut ~[Option<T>]) {
    ...
    /* fill with None */
    foreach _ in Range::new(elts.len(), elts.capacity()) {
        elts.push(None);
    }
    ...
    if lo <= (nelts - lo) { // A
        foreach i in Range::new(0u, lo) {
            elts.swap(i, nelts + i);
        }
    } else {                // B
        foreach i in Range::new(lo, nelts) {
...
mod tests {
    ...
    fn test_iter() {
        ...
        foreach i in Range::new(0, 5) {
            d.push_back(i);
        }
        assert_eq!(d.iter().collect::<~[&int]>(), ~[&0,&1,&2,&3,&4]);

        foreach i in Range::new(6, 9) {
            d.push_front(i);
        }
