...
impl BigBitv {
    ...
    pub fn each_storage(&mut self, op: &fn(v: &mut uint) -> bool) -> bool {
        Range::new(0u, self.storage.len()).advance(|i| op(&mut self.storage[i]))
    }
...
impl Bitv {
    ...
    */
    ...
    pub fn ones(&self, f: &fn(uint) -> bool) -> bool {
        Range::new(0u, self.nbits).advance(|i| !self.get(i) || f(i))
    }
...
fn iterate_bits(base: uint, bits: uint, f: &fn(uint) -> bool) -> bool {
    ...
    foreach i in Range::new(0u, uint::bits) {
        if bits & (1 << i) != 0 {
            if !f(base + i) {
                return false;
            }
...
impl BitvSet {
    ...
    fn other_op(&mut self, other: &BitvSet, f: &fn(uint, uint) -> uint) {
        fn nbits(mut w: uint) -> uint {
            let mut bits = 0;
            foreach _ in Range::new(0u, uint::bits) {
                if w == 0 {
                    break;
                }
                bits += w & 1;
