...
fn scrypt_ro_mix(b: &mut [u8], v: &mut [u8], t: &mut [u8], n: uint) {
    fn integerify(x: &[u8], n: uint) -> uint {
        ...
        // this won't overflow since we've already checked that values mod n fit into a uint.
...
impl ScryptParams {
     ...
     *
     */
    pub fn new(log_n: uint, r: uint, p: uint) -> ScryptParams {
        ...
        // Ensure that (1 << log_n) fits inside a uint
        assert!(log_n < size_of::<uint>() * 8);

        // check: n < 2^(128 * r / 8)
        let n_check = match r.checked_mul(&16) {
            Some(x) => x,
            None => fail!("Invalid Scrypt parameters.")
        };
        assert!(log_n < n_check);
        ...
        assert!(rp <= max / 128); // ie: r * p * 128 < max
        assert!(r <= (max / 128) >> log_n); // ie: r * n * 128 < max
...
pub fn scrypt_simple(password: &str, params: &ScryptParams) -> ~str {
    // TODO - Some sort of comment about this!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if params.r < 256 && params.p < 256 {
        ...
        result.push_str("1$");
        let mut tmp = [0u8, ..9];
        // TODO - Check that these casts are valid!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        tmp[0] = params.log_n as u8;
        write_u32_le(tmp.mut_slice(1, 5), params.r as u32);
        write_u32_le(tmp.mut_slice(5, 9), params.p as u32);
        result.push_str(tmp.to_base64(base64::STANDARD));
    }
    result.push_char('$');
        ...
        Some(fstr) => {
            ...
            match fstr {
                "0" => {
                    ...
                    // TODO - Check that these casts are valid!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    let log_n = pvec[0] as uint;
                    let r = pvec[1] as uint;
                    let p = pvec[2] as uint;
                ...
                "1" => {
                    ...
                    params = ScryptParams::new(log_n, pval[0] as uint, pval[1] as uint);
                }
...
mod test {
    ...
    fn test_scrypt_simple(log_n: uint, r: uint, p: uint) {
